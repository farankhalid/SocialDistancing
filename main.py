...
def cvBoxes(detections, img):
    ...
    if len(detections) > 0:  # At least 1 detection in the image and check detection presence in a frame
        ...
        for detection in detections:  # In this if statement, we filter all the detections for persons only
            ...
            if name_tag == 'person':
                ...
                xmin, ymin, xmax, ymax = convertBack(float(x), float(y), float(w), float(
                    h))  # Convert from center coordinates to rectangular coordinates, We use floats to ensure the
                # precision of the BBox
                # Append center point of bbox for persons detected.
                centroid_dict[objectId] = (int(x), int(y), xmin, ymin, xmax,
                                           ymax)  # Create dictionary of tuple with 'objectId' as the index center
                # points and bbox
        ...
        for (id1, p1), (id2, p2) in combinations(centroid_dict.items(),
                                                 2):  # Get all the combinations of close detections, #List of
            # multiple items - id1 1, points 2, 1,3
...
def YOLO():
    ...
    configPath = "input/yolov4.cfg"
    weightPath = "input/yolov4.weights"
    metaPath = "input/coco.names"
    ...
    if netMain is None:
        ...
        netMain.setPreferableTarget(cv2.dnn.DNN_TARGET_CUDA_FP16)
        model = cv2.dnn_DetectionModel(netMain)
    ...
    cap = cv2.VideoCapture("rtsp://admin:admin123@192.168.100.9:554/cam/realmonitor?channel=2&subtype=0")
